...
class UpdateGroupInteractor:UpdateGroupBusinessLogic,UpdateGroupDataStore
{
    ...
    func fetchItems(request: UpdateGroupModel.Fetch.Request) {
        ...
        worker!.fetch(user_id:request.user_id!,group_title:request.group_title!,group_lead_id:request.group_lead_id!, status:request.status!, group_id:request.group_id!, dynamic_db:request.dynamic_db!,onSuccess: { (resp) in

...
class ChangePasswordWorker{
   ...
   func fetch(user_id:String,old_password:String,password:String,onSuccess successCallback:(ChangePasswordresponseHandler)?,onFailure failureCallback: @escaping(ChangePasswordresponseHandler)) {
       let manager = APIManager()
       manager.callAPIChangePassword(
        user_id:user_id, old_password:old_password,password:password,onSuccess: { (resp)  in

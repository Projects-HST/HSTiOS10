...
class AddGroupMemberInteractor:AddGroupMemberBusinessLogic, AddGroupMemberDataStore
{
    ...
    func fetchItems(request: AddGroupMemberModel.Fetch.Request) {
        ...
        worker!.fetch(user_id:request.user_id!, group_member_id:request.group_member_id!,group_user_type:request.group_user_type!, status:request.status!,group_id:request.group_id!, onSuccess: { (resp) in

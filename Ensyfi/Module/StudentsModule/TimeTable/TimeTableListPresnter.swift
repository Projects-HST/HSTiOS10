  ...
  weak var viewController1: TimeTableListDisplayLogic?
    ...
    func presentFetchResults(resp: TimeTableListModel.Fetch.Response) {
        ...
        for data in resp.testObj {
                ...
                let displayedTimeTableListDatas = TimeTableListModel.Fetch.ViewModel.DisplayedTimeTableListData(to_time: data.to_time!, day_id: data.day_id!,teacher_id: data.teacher_id!, class_name: data.class_name!, break_name: data.break_name!,from_time: data.from_time!, subject_id: data.subject_id!,is_break: data.is_break!, table_id: data.table_id!,period: data.period!, class_id: data.class_id!,sec_name: data.sec_name!)

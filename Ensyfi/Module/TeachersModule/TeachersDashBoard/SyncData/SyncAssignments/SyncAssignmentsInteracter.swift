...
class SyncAssignmentInteractor:SyncAssignmentBusinessLogic,SyncAssignmentDataStore
{
    ...
    func fetchItems(request: SyncAssignmentModel.Fetch.Request) {
        ...
        worker!.fetch(class_id:request.class_id!, teacher_id:request.teacher_id!,homeWork_type:request.homeWork_type!, subject_id:request.subject_id!, title:request.title!,test_date:request.test_date!, due_date:request.due_date!,homework_details:request.homework_details!, created_by:request.created_by!,  created_at:request.created_at!,dynamic_db:request.dynamic_db!,onSuccess: { (resp) in
